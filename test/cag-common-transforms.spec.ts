...
function compactCagSide(cagSide: any) {
  return {
    pos: [
...
function compareCagSides(t: any, expected: any, observed: any, precision: any) {
  nearlyEqual(t, expected.pos[0][0], observed.pos[0][0], precision);
  nearlyEqual(t, expected.pos[0][1], observed.pos[0][1], precision);
}

function getSimplifiedVertexPosition(vertex: any) {
...
test('CAG should transform properly', t => {
  ...
  const res1 = cag1.transform(matrixA);
