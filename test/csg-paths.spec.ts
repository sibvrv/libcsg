...
test('Path2D constructor creates an empty path', t => {
  const p1 = new Path2D();
...
test('Path2D.arc() creates correct paths', t => {
// test default options
  ...
  t.false(a1.isClosed());
  t.is(p1.length, 34);
  t.deepEqual(p1[0], new Vector2([1, 0]));

// test center
  const a2 = Path2D.arc({center: [5, 5]});
  const p2 = a2.getPoints();

  t.false(a2.isClosed());
  t.is(p2.length, 34);
  t.deepEqual(p2[0], new Vector2([6, 5]));

// test radius (with center)
  const a3 = Path2D.arc({center: [5, 5], radius: 10});
  const p3 = a3.getPoints();

  t.false(a3.isClosed());
  t.is(p3.length, 34);
  t.deepEqual(p3[0], new Vector2([15, 5]));

// test start angle (with radius)
  const a4 = Path2D.arc({radius: 10, startangle: 180});
  const p4 = a4.getPoints();

  t.false(a4.isClosed());
  t.is(p4.length, 18);
  // t.deepEqual(p4[0],new Vector2([10,0]))

// test end angle (with center)
  const a5 = Path2D.arc({center: [5, 5], endangle: 90});
  const p5 = a5.getPoints();

  t.false(a5.isClosed());
  t.is(p5.length, 10);
  t.deepEqual(p5[0], new Vector2([6, 5]));

// test resolution (with radius)
  const a6 = Path2D.arc({radius: 10, resolution: 144});
  const p6 = a6.getPoints();

  t.false(a6.isClosed());
  t.is(p6.length, 146);
  t.deepEqual(p6[0], new Vector2([10, 0]));

// test make tangent (with radius)
  const a7 = Path2D.arc({radius: 10, maketangent: true});
  const p7 = a7.getPoints();

  t.false(a7.isClosed());
  t.is(p7.length, 36);
  t.deepEqual(p7[0], new Vector2([10, 0]));
});

test('Path2D creates CAG from paths', t => {
  let p1 = new Path2D([[27.5, -22.96875]], false);
  ...
  let p2 = new Path2D([[27.5, -22.96875]], false);
  ...
  let p3 = new Path2D([[27.5, -22.96875]], false);
  ...
  let p4 = new Path2D([[27.5, -22.96875]], false);
  ...
  let p5 = new Path2D([[10, -20]], false);
  p5 = p5.appendBezier([[10, -10], [25, -10], [25, -20]]);
  p5 = p5.appendBezier([[25, -30], [40, -30], [40, -20]]);

  const cag05 = p5.expandToCAG(0.05, defaultResolution2D);
  t.is(typeof cag05, 'object');
});

test('Path2D closed cw square turns cw', t => {
  const path = new Path2D([[0, 0]], false).appendPoint([1, 0]).appendPoint([1, 1]).appendPoint([0, 1]).close();
  t.deepEqual(path.getTurn(), 'clockwise');
});

test('Path2D unclosed cw triangle turns cw', t => {
  const path = new Path2D([[0, 0]], false).appendPoint([1, 0]).appendPoint([0, 1]);
  t.deepEqual(path.getTurn(), 'clockwise');
});

test('Path2D closed ccw square is ccw', t => {
  const path = new Path2D([[0, 0]], false).appendPoint([0, 1]).appendPoint([1, 1]).appendPoint([1, 0]).close();
  t.deepEqual(path.getTurn(), 'counter-clockwise');
});

test('Path2D unit straight path area is straight', t => {
  const path = new Path2D([[0, 0]], false).appendPoint([0, 1]);
