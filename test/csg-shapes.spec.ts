...
test('CSG.cube creates a cube', t => {
  ...
  const c4 = CSG.cube({center: [5, 5, 5], radius: [5, 10, 20]});
  const f1 = c4.getFeatures('volume');
  t.is(f1, 8000);
  const f2 = c4.getFeatures('area');
  t.is(f2, 2800);
  const f3 = c4.getFeatures(['volume', 'area']);
  t.is(f3[0], 8000);
  t.is(f3[1], 2800);
});
...
test('CSG should produce proper polyhedrons', t => {
  ...
  isValid(t, 'pol1', CSG.polyhedron({points, faces}));
});
...
function getSimpleSides(cag: any) {
  return cag.sides.map((side: any) => {
