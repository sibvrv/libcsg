...
// Note: points are created CCW about the Z (normal)
  ...
  const p = observed.vertices[1].pos;
...
test('CSG.Polygon3 transformations', t => {
  ...
  const flipped = original.flipped();
  const fplane = original.plane.flipped();
...
test('CSG.Polygon3 conversions to CAG CSG', t => {
  ...
  const plane = new CSG.Plane(new Vector3(0, 0, 1), 0);
  const onb = new CSG.OrthoNormalBasis(plane);

  const acag = original.projectToOrthoNormalBasis(onb);

  t.deepEqual(acag, CAG.fromPointsNoCheck([[0, 10], [0, 0], [10, 10]]));

  const acsg = original.extrude(new Vector3(0, 0, 20));

  t.is(acsg.polygons.length, 5);
  t.deepEqual(acsg.polygons[0], original);

  const tpoly = acsg.polygons[4];
...
test('CSG.Polygon.Shared construction', t => {
  const s1 = new CSG.Polygon.Shared([1, 2, 3, 4]);

  t.deepEqual(s1.color, [1, 2, 3, 4]);

  const o2 = {color: [4, 3, 2, 1]};
  const s2 = CSG.Polygon.Shared.fromObject(o2);

  t.deepEqual(s2.color, [4, 3, 2, 1]);

  const s3 = CSG.Polygon.Shared.fromColor(9, 8, 7, 6);

  t.deepEqual(s3.color, [9, 8, 7, 6]);

  const s4 = CSG.Polygon.Shared.fromColor([6, 7, 8, 9]);

  t.deepEqual(s4.color, [6, 7, 8, 9]);

// check that generic objects are possible via JSON
  const o5 = JSON.parse(JSON.stringify(s4));
  const s5 = CSG.Polygon.Shared.fromObject(o5);

  t.deepEqual(o5, JSON.parse(JSON.stringify(s5)));

  const s6 = CSG.Polygon.Shared.fromColor([4, 3, 2]);

  t.deepEqual(s6.color, [4, 3, 2, 1]);

// check that fromObject works without a color
  const o7 = new CSG.Polygon.Shared();
  const s7 = CSG.Polygon.Shared.fromObject(o7);
