import {vector_char, vector_text, vectorChar, vectorText} from '../';
import {expect} from 'chai';
import {TEXT_ALIGN} from '../vectorParams';
...
describe('Vector Text', () => {
  it('vector_char (x, y, char)', () => {
    const obs = vector_char(0, 0, '?');

    expect(obs.width).eq(18);
    expect(obs.height).eq(14);
    expect(obs.segments).deep.eq(questionMarkSegments);
  });

  it('vector_text (x, y, text)', () => {
    const obs = vector_text(0, 0, 'Open');

    expect(obs).deep.eq(openTextSegments);
  });

  it('vectorChar (defaults)', () => {
  ...
  it('vectorChar (char)', () => {
    ...
    expect(obs.width).eq(22);
    expect(obs.height).eq(14);
    expect(obs.segments).deep.eq(expSegments);
  });

  it('vectorChar ({ xOffset, yOffset }, char)', () => {
  ...
  it('vectorChar ({ height }, char)', () => {
    ...
    expect(obs.width).eq(13.571428571428571);
    expect(obs.height).eq(10);
    expect(obs.segments).deep.eq(expSegments);
  });
  ...
  it('vectorText (text)', () => {
    ...
    expect(obs).deep.eq(expSegments);
  });

  it('vectorText (multi-line-text)', () => {
  ...
  it('vectorText ({ xOffset, input, letterSpacing })', () => {
  ...
  it('vectorText ({ height, extrudeOffset }, text)', () => {
  ...
  it('vectorText ({ align: center }, text)', () => {
    const obs = vectorText({align: TEXT_ALIGN.CENTER}, 'a\nab\nabc');
    const expSegments = [
      [[33.5, 14], [33.5, 0]],
  ...
  it('vectorText ({ align: right }, text)', () => {
    const obs = vectorText({align: TEXT_ALIGN.RIGHT}, 'a\nab\nabc');
    const expSegments = [
      [[52, 14], [52, 0]],
